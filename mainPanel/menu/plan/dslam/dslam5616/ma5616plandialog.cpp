#include "ma5616plandialog.h"
#include "ui_ma5616plandialog.h"
#include "lib/database/danetdbman.h"
#include <QSqlQuery>
#include <QMessageBox>
#include <QRegExpValidator>

MA5616PlanDialog::MA5616PlanDialog(QWidget *parent, DanetDbMan *Db, const int ExchId, const int DslamId, const QString Name) :
    QDialog(parent),
    ui(new Ui::MA5616PlanDialog),
    dbMan(Db),
    exchId(ExchId),
    dslamId(DslamId),
    name(Name),
...
//    ui->ipMediaLE->setValidator(ipMediaValidator);
    ...
    bool dslamExist = dbMan->dslamExistance(dslamId);


    QString purchase = dbMan->getDevexPurchase(dslamId);
    ...
    QList<int> ipIds = dbMan->getDslamNmsIpIds(dslamId);
    int gIpId = -1, mIpId = -1;
    if(ipIds.count() > 1)
    {
        gIpId = ipIds.at(0);
    ...
    if(node == 0)
    {
        ...
        //        int vlan = dbMan->getNewPppoeVlan(devIds,3);
        //        if(vlan == -1)
...
//        if(dbMan->dslamAggExistance(dslamId) || dbMan->dslamPppoeExistance(dslamId))
//        {
...
MA5616PlanDialog::~MA5616PlanDialog()
{
    delete ui;
}

void MA5616PlanDialog::entryCheck()
{
    if(entriesCheck())
...
bool MA5616PlanDialog::entriesCheck()
{
    ...
    if(  hsi || inet || sinet || voip ) // btv || ipmedia ||
        ui->dslamNextBtn->setEnabled(false);
    else
        ui->dslamNextBtn->setEnabled(true);
...
bool MA5616PlanDialog::insertVlan(QString vlanRanges, QString desc)
{
    //dslam
    ...
    {
        if(!dbMan->insertVlan(dslamId,vlan, desc))
            return false;
    }
...
bool MA5616PlanDialog::insertVlan(int vlan, QString desc)
{
    if(dbMan->insertVlan(dslamId,vlan, desc))
        return true;
    else
...
void MA5616PlanDialog::fillDslamParam()
{
    int SN = 1, TYPE_NO = 1;
    ...
    QList<int>  hsiList, inetList, sinetList, voipList;//, ngnList;btvList, ipMediaList,
...
//    ui->ipMediaLE->setText(temp);
    ...
    // `id`, `port`, `description`
    QString port, desc;
    ui->desc0LE->blockSignals(true);
    ui->desc1LE->blockSignals(true);
    ...
    while(query->next())
    {
        ...
        if(port.compare("1G-0/0/0", Qt::CaseInsensitive) == 0)
            ui->desc0LE->setText(desc);
        else
            if(port.compare("1G-0/0/1", Qt::CaseInsensitive) == 0)
                ui->desc1LE->setText(desc);
    }

    ui->desc0LE->blockSignals(false);
    ui->desc1LE->blockSignals(false);

}

void MA5616PlanDialog::on_typeCB_currentIndexChanged(int index)
{
    index++;
...
void MA5616PlanDialog::on_snSB_valueChanged(int arg1)
{
    arg1++;
    dslamNameChanged = true;
    entryCheck();
...
void MA5616PlanDialog::on_gIppCB_currentTextChanged(const QString &arg1)
{
    ipChanged =true;
    if(arg1.isEmpty())
    {
...
void MA5616PlanDialog::on_mIppCB_currentTextChanged(const QString &arg1)
{
    ipChanged =true;
    if(arg1.isEmpty())
    {
...
void MA5616PlanDialog::on_desc0LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void MA5616PlanDialog::on_desc1LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void MA5616PlanDialog::on_okBtn_clicked()
{
    if(ExistPlan)
    {
        if(dslamNameChanged)
        {
            ...
            if(dbMan->insertUpdateDslam(dslamId, type, SN))
            {
                QString dslamName = dbMan->getDslamName(dslamId);
                //update agg desc
                if(!dbMan->updateAggDslamDesc(dslamId, dslamName))
                    QMessageBox::warning(this, "ERROR", "Cannot update Agg-DSLAM description.\n"+dbMan->lastError());

                //update cx desc
                if(!dbMan->updateCxDslamDesc(dslamId, dslamName))
                    QMessageBox::warning(this, "ERROR", "Cannot update CX-DSLAM description.\n"+dbMan->lastError());

                //update bras
                QString device = dbMan->getDslamDevice(dslamId); // EQ-T-P-SN
                int agg1Id = dbMan->getDslamAgg1Id(dslamId);
                QString agg = dbMan->getAggNameBrief(agg1Id, true);// NN-S9303-AGG#
                if(!dbMan->updateDslamPppoe(dslamId,agg,device))
        ...
        {
            //release  NMS ip
            QList<int> ipPoolId = dbMan->getDslamIpPoolId(dslamId);
            int gIpPId, mIpPId;
            if(ipPoolId.count() > 1)
            {
            ...
            if(! dbMan->updateDslamNms(dslamId,gVlan,gIpPoolId,gIpp,gBind,gGw,mVlan,mIpPoolId, mIpp,mBind,mGw, assignDesc))
            {
        ...
        if(descChanged)
        {
            ...
            if(!dbMan->deleteDslamInterface(dslamId))
            {
            ...
            QString port = "1G-0/0/0";
            QString desc = ui->desc0LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(dslamId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());


            port = "1G-0/0/1";
            desc = ui->desc1LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(dslamId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());
...
//        ui->ipMediaLE->setText(range);
        ...
        if( (ui->uplinkShelfCB->isChecked() && node == 0) || (node == 1) )
        {
            range = ui->hsiLE->text();
            list = dbMan->rangeToList(range);
            QList<int> devIds = dbMan->getExchangeSiteAllDslamIds(motherId);
...
//            }
        ...
        if(dbMan->insertUpdateDslam(dslamId, type, SN))
        {
            ...
            if(!dbMan->deleteDslamParams(dslamId))
            {
...
//            }
            ...
            QString assignDesc = name+"- "+type+"- "+purchase+"- "+QString::number(SN);
            if(! dbMan->insertDslamNms(dslamId,gVlan,gIpPoolId,gIpp,gBind,gGw,mVlan,mIpPoolId, mIpp,mBind,mGw, assignDesc))
            {
        ...
        if(ui->desc0LE->text().isEmpty())
        {
            ...
            if(uplinkSiteId > -1)
            {
                ...
                if(uplinkDevexName.contains("5616"))
                {
                    ...
                    ok = ok && dbMan->insertUpdateDslamInterface(dslamId,"1G-0/0/0", uplinkDevexName);
                    ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/0/1", name+" 1G-0/0/0");
                ...
                if(uplinkDevexName.contains("5600"))
                {
                    ...
                    if(dbMan->dslamInterfaceExistence(dslamId, "1G-0/7/4"))
                    {
                        uplinkDevexName +=" 1G-0/7/4";
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/7/4", name+" 1G-0/0/0");

                    }
                    else
                    {
                        uplinkDevexName +=" 1G-0/7/5";
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/7/5", name+" 1G-0/0/0");
...
void MA5616PlanDialog::on_uplinkShelfCB_toggled(bool checked)
{
    if(checked)
    {
        //ui->vasLE->setPlaceholderText("100-105, 105, 125");

        //ui->ipMediaLE->setPlaceholderText("300-305, 305, 325");

        ui->hsiLE->setPlaceholderText("500-505, 505, 525");

        ui->inetLE->setPlaceholderText("600-605, 605, 625");

        ui->sinetLE->setPlaceholderText("700-705, 705, 725");

        ui->voipLE->setPlaceholderText("800-805, 805, 825");
...
void MA5616PlanDialog::on_gIppCB_currentIndexChanged(int index)
{
    ui->mIppCB->setCurrentIndex(index);
}

//void MA5616PlanDialog::on_btvLE_textChanged(const QString &arg1)
//{
//    ui->okBtn->setEnabled(false);
//    ui->dslamNextBtn->setEnabled(false);
...
//void MA5616PlanDialog::on_ipMediaLE_textChanged(const QString &arg1)
//{
//    ui->okBtn->setEnabled(false);
//    ui->dslamNextBtn->setEnabled(false);
...
void MA5616PlanDialog::on_hsiLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNextBtn->setEnabled(false);
...
void MA5616PlanDialog::on_inetLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNextBtn->setEnabled(false);
...
void MA5616PlanDialog::on_sinetLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNextBtn->setEnabled(false);
...
void MA5616PlanDialog::on_voipLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNextBtn->setEnabled(false);
...
//void MA5616PlanDialog::on_ngnLE_textChanged(const QString &arg1)
//{
//    ui->okBtn->setEnabled(false);
...
void MA5616PlanDialog::on_dslamNextBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(1);
}

void MA5616PlanDialog::on_descBackBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(1);
}

void MA5616PlanDialog::on_nmsNextBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(2);
}

void MA5616PlanDialog::on_nmsBackBtn_clicked()
{
