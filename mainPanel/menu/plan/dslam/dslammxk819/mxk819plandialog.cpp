#include "mxk819plandialog.h"
#include "ui_mxk819plandialog.h"
#include "lib/database/danetdbman.h"
#include <QSqlQuery>
#include <QMessageBox>
#include <QRegExpValidator>

Mxk819PlanDialog::Mxk819PlanDialog(QWidget *parent, DanetDbMan *Db, const int ExchId, const int DevexId, const QString Name) :
    QDialog(parent),
    ui(new Ui::Mxk819PlanDialog),
    dbMan(Db),
    exchId(ExchId),
    devexId(DevexId),
    name(Name),
...
*/
    ...
    else if( (node > 1) || (shelf > 1))
    {
        ...
        int siteId = dbMan->getExchangeSiteId(motherId, cascade, node-1);
        QList<int> dslamDevexIds = dbMan->getExchangeSiteDslamIds(siteId);
        ...
        if(dslamDevexIds.count() >= 1)
        {
            ...
            QList<int> hsiList = dbMan->getDslamPassedPppoeVlanList(node1devexId);
...
//        {
    ...
    else
    {
        ...
        if((uplinkDevexId != devexId) && (uplinkDevexId > -1))
        {
            ...
            if(currentShelf == 2)
            {
                uplinkShelfName += "- 1G 1-a-8-0/eth";
                ui->desc1a2LE->setText(uplinkShelfName);
            }
            else if(currentShelf == 3)
            {
                uplinkShelfName += "- 1G 1-a-9-0/eth";
                ui->desc1a2LE->setText(uplinkShelfName);
            }
        }
    }
...
Mxk819PlanDialog::~Mxk819PlanDialog()
{
    delete ui;
}

void Mxk819PlanDialog::entryCheck()
{
    if(entriesCheck())
...
bool Mxk819PlanDialog::entriesCheck()
{
    ...
    if( hsi || inet || sinet || voip ) //btv || ipmedia ||
        ui->dslamNext->setEnabled(false);
    else
        ui->dslamNext->setEnabled(true);
    ...
    if(t || gIp || gGw || gBind || mBind || mIp || mGw || hsi || inet || sinet || voip )// || btv || ipmedia
        return false;
    else
        return true;
}

bool Mxk819PlanDialog::insertVlan(QString vlanRanges, QString desc)
{
    //dslam
...
bool Mxk819PlanDialog::insertVlan(int vlan, QString desc)
{
    if(dbMan->insertVlan(devexId,vlan, desc))
        return true;
    else
...
void Mxk819PlanDialog::fillDslamParam()
{
    int SN = 1, TYPE_NO = 0;
...
//    ui->ipMediaLE->setText(temp);
    ...
    // `id`, `port`, `description`
    QString port, desc;
    ui->desc1a2LE->blockSignals(true);
    ui->desc1a3LE->blockSignals(true);
    ui->desc1a4LE->blockSignals(true);
    ui->desc1a5LE->blockSignals(true);
    ui->desc1a6LE->blockSignals(true);
    ui->desc1a7LE->blockSignals(true);
    ui->desc1a8LE->blockSignals(true);
    ui->desc1a9LE->blockSignals(true);
    ui->desc1a10LE->blockSignals(true);
    ui->desc1a11LE->blockSignals(true);

    ui->desc1b2LE->blockSignals(true);
    ui->desc1b3LE->blockSignals(true);
    ui->desc1b4LE->blockSignals(true);
    ui->desc1a5LE->blockSignals(true);
    ui->desc1b6LE->blockSignals(true);
    ui->desc1b7LE->blockSignals(true);
    ui->desc1b8LE->blockSignals(true);
    ui->desc1b9LE->blockSignals(true);
    ui->desc1b10LE->blockSignals(true);
    ui->desc1b11LE->blockSignals(true);
    ...
    while(query->next())
    {
        ...
        if(port.compare("1G 1-a-2-0/eth", Qt::CaseInsensitive) == 0)
            ui->desc1a2LE->setText(desc);
        else
            if(port.compare("1G 1-a-3-0/eth", Qt::CaseInsensitive) == 0)
                ui->desc1a3LE->setText(desc);
            else
                if(port.compare("1G 1-a-4-0/eth", Qt::CaseInsensitive) == 0)
                    ui->desc1a4LE->setText(desc);
                else
                    if(port.compare("1G 1-a-5-0/eth", Qt::CaseInsensitive) == 0)
                        ui->desc1a5LE->setText(desc);
                    else
                        if(port.compare("1G 1-a-6-0/eth", Qt::CaseInsensitive) == 0)
                            ui->desc1a6LE->setText(desc);
                        else
                            if(port.compare("1G 1-a-7-0/eth", Qt::CaseInsensitive) == 0)
                                ui->desc1a7LE->setText(desc);
                            else
                                if(port.compare("1G 1-a-8-0/eth", Qt::CaseInsensitive) == 0)
                                    ui->desc1a8LE->setText(desc);
                                else
                                    if(port.compare("1G 1-a-9-0/eth", Qt::CaseInsensitive) == 0)
                                        ui->desc1a9LE->setText(desc);
                                    else
                                        if(port.compare("1G 1-a-10-0/eth", Qt::CaseInsensitive) == 0)
                                            ui->desc1a10LE->setText(desc);

                                        else
                                            if(port.compare("1G 1-a-11-0/eth", Qt::CaseInsensitive) == 0)
                                                ui->desc1a11LE->setText(desc);

        else  if(port.compare("1G 1-b-2-0/eth", Qt::CaseInsensitive) == 0)
                                                ui->desc1b2LE->setText(desc);
                                            else
                                                if(port.compare("1G 1-b-3-0/eth", Qt::CaseInsensitive) == 0)
                                                    ui->desc1b3LE->setText(desc);
                                                else
                                                    if(port.compare("1G 1-b-4-0/eth", Qt::CaseInsensitive) == 0)
                                                        ui->desc1b4LE->setText(desc);
                                                    else
                                                        if(port.compare("1G 1-b-5-0/eth", Qt::CaseInsensitive) == 0)
                                                            ui->desc1b5LE->setText(desc);
                                                        else
                                                            if(port.compare("1G 1-b-6-0/eth", Qt::CaseInsensitive) == 0)
                                                                ui->desc1b6LE->setText(desc);
                                                            else
                                                                if(port.compare("1G 1-b-7-0/eth", Qt::CaseInsensitive) == 0)
                                                                    ui->desc1b7LE->setText(desc);
                                                                else
                                                                    if(port.compare("1G 1-b-8-0/eth", Qt::CaseInsensitive) == 0)
                                                                        ui->desc1b8LE->setText(desc);
                                                                    else
                                                                        if(port.compare("1G 1-b-9-0/eth", Qt::CaseInsensitive) == 0)
                                                                            ui->desc1b9LE->setText(desc);
                                                                        else
                                                                            if(port.compare("1G 1-b-10-0/eth", Qt::CaseInsensitive) == 0)
                                                                                ui->desc1b10LE->setText(desc);

                                                                            else
                                                                                if(port.compare("1G 1-b-11-0/eth", Qt::CaseInsensitive) == 0)
                                                                                    ui->desc1b11LE->setText(desc);


    }

    ui->desc1a2LE->blockSignals(false);
    ui->desc1a3LE->blockSignals(false);
    ui->desc1a4LE->blockSignals(false);
    ui->desc1a5LE->blockSignals(false);
    ui->desc1a6LE->blockSignals(false);
    ui->desc1a7LE->blockSignals(false);
    ui->desc1a8LE->blockSignals(false);
    ui->desc1a9LE->blockSignals(false);
    ui->desc1a10LE->blockSignals(false);
    ui->desc1a11LE->blockSignals(false);

}

void Mxk819PlanDialog::on_okBtn_clicked()
{
    if(ExistPlan)
    {
        if(dslamNameChanged)
        ...
        if(descChanged)
        {
            ...
            port = "1G 1-a-9-0/eth";
            desc = ui->desc1a9LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());

            port = "1G 1-a-10-0/eth";
            desc = ui->desc1a10LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());

            port = "1G 1-a-11-0/eth";
            desc = ui->desc1a11LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());
            ...
            port = "1G 1-b-2-0/eth";
            desc = ui->desc1b2LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());

            port = "1G 1-b-3-0/eth";
            desc = ui->desc1b3LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());

            port = "1G 1-b-4-0/eth";
            desc = ui->desc1b4LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());
            ...
            port = "1G 1-b-9-0/eth";
            desc = ui->desc1b9LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());

            port = "1G 1-b-10-0/eth";
            desc = ui->desc1b10LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());

            port = "1G 1-b-11-0/eth";
            desc = ui->desc1b11LE->text();
            if(!desc.isEmpty())
                if(!dbMan->insertDslamInterface(devexId, port, desc))
                    QMessageBox::warning(this, "ERROR", "Cannot insert DSLAM port description.\n"+dbMan->lastError());
...
//            }
            ...
            if((devexId != uplinkDevexId) && (uplinkDevexId > -1))
            {
                ...
                if(currentShelf == 2)
                {
                    dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G 1-a-8-0/eth", dn);
                }
                else if(currentShelf == 3)
                {
                    dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G 1-a-9-0/eth", dn);
                }
        ...
        if(ui->desc1a8LE->text().isEmpty())
        {
            ...
            if(uplinkSiteId > -1)
            {
                ...
                if(uplinkDevexName.contains("5616"))
                {
                    ...
                    ok = ok && dbMan->insertUpdateDslamInterface(devexId,"1G 1-a-2-0/eth", uplinkDevexName);
                    ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/0/1", name+" 1G 1-a-2-0/eth");
                ...
                {
                    ok = ok && dbMan->insertUpdateDslamInterface(devexId,"1G 1-a-2-0/eth", uplinkDevexName);
                    ok = ok && dbMan->insertUpdateDslamInterface(devexId,"1G 1-b-2-0/eth", uplinkDevexName);


                    if(!dbMan->dslamInterfaceExistence(uplinkDevexId, "1G-0/7/4"))
                    {
                        uplinkDevexName +=" 1G-0/7/4";
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/7/4", name+" 1G 1-a-2-0/eth");

                        uplinkDevexName +=" 1G-0/8/4";
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/8/4", name+" 1G 1-b-2-0/eth");

                    }
                    else
                    {
                        uplinkDevexName +=" 1G-0/7/5";
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/7/5", name+" 1G 1-a-2-0/eth");

                        uplinkDevexName +=" 1G-0/8/5";
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G-0/8/5", name+" 1G 1-b-2-0/eth");
                    }
                ...
                if(uplinkDevexName.contains("819"))
                {
                    ok = ok && dbMan->insertUpdateDslamInterface(devexId,"1G 1-a-2-0/eth", uplinkDevexName);
                    ok = ok && dbMan->insertUpdateDslamInterface(devexId,"1G 1-b-2-0/eth", uplinkDevexName);


                    if(!dbMan->dslamInterfaceExistence(uplinkDevexId, "1G 1-a-8-0/eth"))
                    {
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G 1-a-8-0/eth", name+" 1G 1-a-2-0/eth");
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G 1-b-8-0/eth", name+" 1G 1-b-2-0/eth");

                    }
                    else
                    {
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G 1-a-9-0/eth", name+" 1G 1-a-2-0/eth");
                        ok = ok && dbMan->insertUpdateDslamInterface(uplinkDevexId,"1G 1-b-9-0/eth", name+" 1G 1-b-2-0/eth");
                    }
...
void Mxk819PlanDialog::on_uplinkShelfCB_toggled(bool checked)
{
    if(checked)
    {
...
void Mxk819PlanDialog::on_typeCB_currentIndexChanged(int index)
{
    index++;
...
void Mxk819PlanDialog::on_gIppCB_currentTextChanged(const QString &arg1)
{
    ipChanged = true;
    if(arg1.isEmpty())
    {
...
void Mxk819PlanDialog::on_mIppCB_currentTextChanged(const QString &arg1)
{
    ipChanged = true;
    if(arg1.isEmpty())
    {
...
void Mxk819PlanDialog::on_desc1a2LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a3LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a4LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a5LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a6LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a7LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a8LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a9LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_snSB_valueChanged(int arg1)
{
    arg1++;
    dslamNameChanged = true;
    entryCheck();
...
void Mxk819PlanDialog::on_gIppCB_currentIndexChanged(int index)
{
    ui->mIppCB->setCurrentIndex(index);
}

//void Mxk819PlanDialog::on_btvLE_textChanged(const QString &arg1)
//{
//    ui->okBtn->setEnabled(false);
//    ui->dslamNext->setEnabled(false);
...
//void Mxk819PlanDialog::on_ipMediaLE_textChanged(const QString &arg1)
//{
//    ui->okBtn->setEnabled(false);
//    ui->dslamNext->setEnabled(false);
...
void Mxk819PlanDialog::on_hsiLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNext->setEnabled(false);
...
void Mxk819PlanDialog::on_inetLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNext->setEnabled(false);
...
void Mxk819PlanDialog::on_sinetLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNext->setEnabled(false);
...
void Mxk819PlanDialog::on_voipLE_textChanged(const QString &arg1)
{
    ui->okBtn->setEnabled(false);
    ui->dslamNext->setEnabled(false);
...
//void Mxk819PlanDialog::on_ngnLE_textChanged(const QString &arg1)
//{
//    ui->okBtn->setEnabled(false);
...
void Mxk819PlanDialog::on_nmsNextBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(2);
}

void Mxk819PlanDialog::on_nmsBackBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(0);
}

void Mxk819PlanDialog::on_dslamNext_clicked()
{
    ui->tabWidget->setCurrentIndex(1);
}

void Mxk819PlanDialog::on_desc7BackBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(1);
}

void Mxk819PlanDialog::on_desc7NextBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(3);
}

void Mxk819PlanDialog::on_desc8BackBtn_clicked()
{
    ui->tabWidget->setCurrentIndex(2);
}

void Mxk819PlanDialog::on_desc1a10LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1a11LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b2LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b3LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b4LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b5LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b6LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b7LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b8LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b9LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b10LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
...
void Mxk819PlanDialog::on_desc1b11LE_textChanged(const QString &arg1)
{
    descChanged = true;
    if(arg1.isEmpty())
        entryCheck();
