#include "subnet1v4assignmentdialog.h"
#include "ui_subnet1v4assignmentdialog.h"
#include "lib/database/danetdbman.h"
#include "lib/netTools/nettools.h"
#include <QMessageBox>
#include <QSqlQuery>

Subnet1V4AssignmentDialog::Subnet1V4AssignmentDialog(QWidget *parent, DanetDbMan *db, const int SubnetPoolId, const QString net, const int prefix) :
    QDialog(parent),
    ui(new Ui::Subnet1V4AssignmentDialog),
    dbMan(db),
    subnetPoolId(SubnetPoolId)
{
    ...
    ui->bcLbl->setText(NetTools::broadcast(net,prefix));
    ...
    //dep
    ui->depCB->blockSignals(true);
    ui->depCB->addItem("Data", 1);
    ui->depCB->addItem("Switch", 2);
    ui->depCB->addItem("Access", 3);
    ui->depCB->addItem("Transmission", 4);
    ui->depCB->setCurrentIndex(-1);
    ui->depCB->blockSignals(false);
...
Subnet1V4AssignmentDialog::Subnet1V4AssignmentDialog(QWidget *parent, DanetDbMan *db, const int SubnetPoolId, const QString net, const int prefix, const int area, const int exchId, const int depNo, const QString ap, const QString desc) :
    QDialog(parent),
    ui(new Ui::Subnet1V4AssignmentDialog),
    dbMan(db),
    subnetPoolId(SubnetPoolId)
...
//exchId == -2 > general
    ...
    ui->depCB->addItem("Access", 3);
    ui->depCB->addItem("Transmission", 4);
    ui->depCB->setCurrentIndex(ui->depCB->findData(depNo));
    ui->depCB->blockSignals(false);
...
Subnet1V4AssignmentDialog::~Subnet1V4AssignmentDialog()
{
    delete ui;
}

void Subnet1V4AssignmentDialog::entryCheck()
{
...
void Subnet1V4AssignmentDialog::on_areaCB_currentIndexChanged(int index)
{
    index++;
    ui->abbrCB->clear();
...
void Subnet1V4AssignmentDialog::on_descLE_textChanged(const QString &arg1)
{
    if(arg1.isEmpty())
        ui->okBtn->setEnabled(false);
    else
...
void Subnet1V4AssignmentDialog::on_cancelBtn_clicked()
{
    this->close();
}

void Subnet1V4AssignmentDialog::on_okBtn_clicked()
{
    int area = ui->areaCB->currentData().toInt();
    int exchId = ui->abbrCB->currentData().toInt();
    int depNo = ui->depCB->currentData().toInt();
    ...
    if(dbMan->updateSubnetPool1(subnetPoolId,area,exchId,depNo,ap,desc))
    {
        dbMan->insertLog(user,"Assign Subnet1: "+Network+" desc: "+desc);
        this->close();
    }
    else
...
void Subnet1V4AssignmentDialog::on_abbrCB_currentIndexChanged(int index)
{
    index++;
    entryCheck();
}

void Subnet1V4AssignmentDialog::on_appCB_currentIndexChanged(int index)
{
    if(index == 0)
    {
...
void Subnet1V4AssignmentDialog::on_appLE_textChanged(const QString &arg1)
{
    if(arg1.isEmpty())
        entryCheck();
    else
...
void Subnet1V4AssignmentDialog::on_depCB_currentIndexChanged(int index)
{
    if(index > -1)
        entryCheck();
    else
