#include "mergesubnet3v4dialog.h"
#include "ui_mergesubnet3v4dialog.h"
#include "lib/database/danetdbman.h"
#include "lib/netTools/nettools.h"
#include <QSqlQuery>
#include <QMessageBox>

MergeSubnet3V4Dialog::MergeSubnet3V4Dialog(QWidget *parent, DanetDbMan *db, const int SubnetPoolId, const QString Abbr, const QString App, const QString Subnet, const int Prefix) :
    QDialog(parent),
    ui(new Ui::MergeSubnet3V4Dialog),
    dbMan(db),
    ...
    prefix(Prefix)
{
    ...
    int ipC = dbMan->getIpPoolCount(subnetPoolId, 3);
    ...
    if(ipC > 0)
    {
        ui->infoLbl->setText("You need to remove related IP pool first.");
    ...
    else
    {
        ...
        else
        {
            //have adjacent
            //get adjacent id
            adjacentSubnetPoolId = dbMan->getSubnetPool3Id(adjacentSubnet, prefix);
            ipC = dbMan->getIpPoolCount(adjacentSubnetPoolId, 3);
            if(ipC > 0)
            {
                // have IP pool
                ui->infoLbl->setText("You need to remove adjacent IP pools first.");
                ui->infoLbl->setVisible(true);
                ui->checkBox->setChecked(false);
                ui->checkBox->setEnabled(false);
                ui->okBtn->setEnabled(false);
            ...
            else
            {
                ...
                {
                    QSqlQuery *query = dbMan->selectSubnetPool3Record(adjacentSubnetPoolId);
...
MergeSubnet3V4Dialog::~MergeSubnet3V4Dialog()
{
    delete ui;
}

void MergeSubnet3V4Dialog::on_checkBox_toggled(bool checked)
{
    if(checked)
...
void MergeSubnet3V4Dialog::on_cancelBtn_clicked()
{
    this->close();
}

void MergeSubnet3V4Dialog::on_okBtn_clicked()
{
    if(dbMan->updateSubnetPool3(subnetPoolId,mSubnet,mPrefix,mMask,mBc,mHost))
    {
        if(dbMan->deleteSubnetPool3(adjacentSubnetPoolId))
        {
            this->close();
        }
        else
        {
            dbMan->updateSubnetPool3(subnetPoolId,subnet,prefix,mask,bc,host);
