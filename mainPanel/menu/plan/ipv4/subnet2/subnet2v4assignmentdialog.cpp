#include "subnet2v4assignmentdialog.h"
#include "ui_subnet2v4assignmentdialog.h"
#include "lib/database/danetdbman.h"
#include "lib/netTools/nettools.h"
#include <QMessageBox>
#include <QSqlQuery>

Subnet2V4AssignmentDialog::Subnet2V4AssignmentDialog(QWidget *parent, DanetDbMan *db, const int SubnetPool2Id, const QString net, const int prefix) :
    QDialog(parent),
    ui(new Ui::Subnet2V4AssignmentDialog),
    dbMan(db),
...
Subnet2V4AssignmentDialog::Subnet2V4AssignmentDialog(QWidget *parent, DanetDbMan *db, const int SubnetPool2Id, const QString net, const int prefix, const int area, const int exchId, const QString ap, const QString desc) :
    QDialog(parent),
    ui(new Ui::Subnet2V4AssignmentDialog),
    dbMan(db),
    subnetPool2Id(SubnetPool2Id)
...
Subnet2V4AssignmentDialog::~Subnet2V4AssignmentDialog()
{
    delete ui;
}

void Subnet2V4AssignmentDialog::entryCheck()
{
...
void Subnet2V4AssignmentDialog::on_areaCB_currentIndexChanged(int index)
{
    index++;
    ui->abbrCB->clear();
...
void Subnet2V4AssignmentDialog::on_descLE_textChanged(const QString &arg1)
{
    if(arg1.isEmpty())
        ui->okBtn->setEnabled(false);
    else
...
void Subnet2V4AssignmentDialog::on_cancelBtn_clicked()
{
    this->close();
}

void Subnet2V4AssignmentDialog::on_okBtn_clicked()
{
    int area = ui->areaCB->currentData().toInt();
    ...
    if(dbMan->updateSubnetPool2(subnetPool2Id,area,exchId,ap,desc))
    {
        dbMan->insertLog(user,"Assign Subnet2: "+Network+" desc: "+desc);
        this->close();
    }
    else
...
void Subnet2V4AssignmentDialog::on_abbrCB_currentIndexChanged(int index)
{
    index++;
    entryCheck();
}

void Subnet2V4AssignmentDialog::on_appCB_currentIndexChanged(int index)
{
    if(index == 0)
    {
...
void Subnet2V4AssignmentDialog::on_appLE_textChanged(const QString &arg1)
{
    if(arg1.isEmpty())
        entryCheck();
    else
