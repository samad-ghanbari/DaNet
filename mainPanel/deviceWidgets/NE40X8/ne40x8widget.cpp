#include "ne40x8widget.h"
#include "ui_ne40x8widget.h"
#include <QSqlQueryModel>

#define UnusedSlotColor "#c9bd43"

NE40X8Widget::NE40X8Widget(QWidget *parent,const bool NE40FLAG, QSqlQueryModel *model) :
    QWidget(parent),
    ui(new Ui::NE40X8Widget)
{
    ui->setupUi(this);
    ...
    if(!NE40FLAG)
        ui->deviceLabel->setText("Huawei CX600X8");
...
NE40X8Widget::~NE40X8Widget()
{
    delete ui;
}

void NE40X8Widget::hideSlots()
{
...
QString NE40X8Widget::stringToVerticalString(QString string)
{
...
void NE40X8Widget::setText(const int slot, int subSlot, QString card)
{
    if(slot == 1)
    {
        if(subSlot == 0)
