#include "ReportFloorPlanTR.h"
#include <QAbstractItemModel>
#include <QDate>
#include <QPainter>
#include <QDebug>

ReportFloorPlanTR::ReportFloorPlanTR(QObject *parent, QAbstractItemModel *Model, const QString Abbr, const QString Exchange, const QString Saloon, const QString OutputPath, const int PaperSize, const int Orient, const QFont Font, const QString CustomInfo) :
    QObject(parent),
...
ReportFloorPlanTR::~ReportFloorPlanTR()
{
    delete painter;
    painter = NULL;
}

void ReportFloorPlanTR::preparePage()
{
    ...
    painter->translate(painter->viewport().width(), headerHeight); // start print point
}

bool ReportFloorPlanTR::print()
{
    if( model->rowCount() == 0 )
    {
...
int ReportFloorPlanTR::getStartRackNo()
{
...
QMap<int, QList<QString> > ReportFloorPlanTR::getrackInfo()
{
...
int ReportFloorPlanTR::odfCount(const int rackRow, const int rackNo)
{
...
void ReportFloorPlanTR::drawOdf(const int rackRow, const int rackNo, const QString device, const int odfNo, const int ppp)
{
...
void ReportFloorPlanTR::drawRack(const int rackRow, const int rackNo, const int shelfCount, const QList<QString> devices)
{
...
void ReportFloorPlanTR::drawRackRow(const int rackRow)
{
    QRect rect(-rightMargin,0, rightMargin, rackHeight);
    painter->save();
...
QString ReportFloorPlanTR::lastError()
{
    return error;
}
