#include "ReportFloorPlanTL.h"
#include <QAbstractItemModel>
#include <QDate>
#include <QPainter>

ReportFloorPlanTL::ReportFloorPlanTL(QObject *parent, QAbstractItemModel *Model, const QString Abbr, const QString Exchange, const QString Saloon, const QString OutputPath, const int PaperSize, const int Orient, const QFont Font, const QString CustomInfo) :
    QObject(parent),
...
ReportFloorPlanTL::~ReportFloorPlanTL()
{
    delete painter;
    painter = NULL;
}

void ReportFloorPlanTL::preparePage()
{
...
bool ReportFloorPlanTL::print()
{
    ...
    while(modelRow < rowCount)
    {
        ...
        if(currentRackRow != row)
        {
            ...
            {
                painter->translate(-leftMargin, rackHeight+50);

                if(painter->viewport().height() < (painter->transform().dy() + rackHeight+50))
                {
                    printer->newPage();
                    preparePage();
                }
...
int ReportFloorPlanTL::getStartRackNo()
{
    int srn = 1;
    int rn, rowCount = model->rowCount();
...
QMap<int, QList<QString> > ReportFloorPlanTL::getrackInfo()
{
...
int ReportFloorPlanTL::odfCount(const int rackRow, const int rackNo)
{
...
void ReportFloorPlanTL::drawRack(const int rackRow, const int rackNo, const int shelfCount, const QList<QString> devices)
{
    int shelfHeight = rackHeight;
    if(shelfCount > 0)
        shelfHeight = rackHeight / shelfCount;

    QRect rect(0,0, rackWidth, shelfHeight);
    ...
    rect.setRect(0,0,rackWidth,25);
    painter->drawText(rect, Qt::AlignCenter, QString::number(rackNo));
    painter->restore();
}

void ReportFloorPlanTL::drawOdf(const int rackRow, const int rackNo,const QString device, const int odfNo, const int ppp)
{
    QRect rect(0,0, odfWidth, rackHeight);
    QString dev;
    ...
    rect.setRect(0,0,odfWidth,25);
    painter->drawText(rect, Qt::AlignCenter, QString::number(rackNo));
    painter->restore();
}

void ReportFloorPlanTL::drawRackRow(const int rackRow)
{

    QRect rect(0,0, leftMargin, rackHeight);
    painter->save();
    ...
    painter->restore();
    painter->translate(leftMargin, 0);
}

QString ReportFloorPlanTL::lastError()
{
    return error;
}
