#include "ReportSubnetIpV4Plan.h"
#include <QAbstractItemModel>
#include <QDate>
#include <QPainter>
#include <QPrinter>

ReportSubnetIpV4Plan::ReportSubnetIpV4Plan(QObject *Parent, const QAbstractItemModel* tableModel, const int poolNo, const QString OutputPath, const QString CustomInfo) :
    QObject(Parent),
    ...
    outputPath(OutputPath)
{
    ...
    if(poolNo == 1)
    {
        columnStretch = QVector<int>() <<0<<15<<8<<15<<15<<8<<0<<8<<0<<10<<15<<0<<15<<15<<30;
        headers = QVector<QString>() << "id" <<"Subnet"<<"Prefix"<< "Mask"<< "Broadcast"<< "Hosts"<< "Area No"<<"Area"<<"Exchange ID"<<"Abbr"<<"Exchange"<<"Department No"<<"Department"<<"Application"<<"Description";
    }
    else
    {
        columnStretch = QVector<int>() <<0<<15<<8<<15<<15<<8<<0<<8<<0<<10<<15<<15<<30;
        headers = QVector<QString>() << "id" <<"Subnet"<<"Prefix"<< "Mask"<< "Broadcast"<< "Hosts"<< "Area No"<<"Area"<<"Exchange ID"<<"Abbr"<<"Exchange"<<"Application"<<"Description";
    }
...
ReportSubnetIpV4Plan::~ReportSubnetIpV4Plan()
{
    delete painter;
    painter = NULL;
}

void ReportSubnetIpV4Plan::preparePage()
{ // print a border on each page
    ...
    painter->drawText(rec,Qt::AlignCenter,"DaNet Subnet Report");
...
bool ReportSubnetIpV4Plan::print()
{
...
bool ReportSubnetIpV4Plan::printTable(QPrinter *printer)
{
...
QString ReportSubnetIpV4Plan::lastError()
{
    return error;
}
